 ...
 */
 ...
 /* --- Send points via PVM --- */
 void send_points(int tid, point *pts) {
	 point *cur;
	 int count = point_nb(pts);
	 pvm_initsend(PvmDataDefault);
	 pvm_pkint(&count, 1, 1);  // number of points
	 for(cur = pts; cur != NULL; cur = cur->next) {
		 int coords[2] = {cur->x, cur->y};
		 pvm_pkint(coords, 2, 1);
	 }
	 pvm_send(tid, 1);
 }
 
 /* --- Receive points via PVM --- */
 point *recv_points(int tid) {
	 int count;
	 int i;
 ...
 /* --- Recursive fully parallel convex hull --- */
 void upper_hull(point *pts) {
	 point *upper, *pts1, *pts2, *res1, *res2;
	 int tids[2];
 
	 upper = point_UH(pts);
	 if (upper) return;  // base case ≤4 points
 
	 // split points
	 pts2 = point_part(pts); // pts2 = second half
	 pts1 = pts;             // first half
 
	 // spawn two slaves
	 int ntask = pvm_spawn("./slave", NULL, PvmTaskDefault, "", 2, tids);
	 int tid = pvm_mytid();
	 if (ntask < 2) {
		 // fallback: compute recursively locally if spawn fails
		 upper_hull(pts1);
		 upper_hull(pts2);
		 point_merge_UH(pts1, pts2);
		 return;
	 }
 
	 // send halves to slaves
	 send_points(tids[0], pts1);
	 send_points(tids[1], pts2);
 
	 // receive results
	 res1 = recv_points(tids[0]);
	 res2 = recv_points(tids[1]);
 
	 // merge
	 point_merge_UH(res1, res2);
 
	 // copy merged result back into pts
	 *pts = *res1;
